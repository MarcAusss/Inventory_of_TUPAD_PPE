<x-po_dashboard_layout title="Assign Call-Off Number">
    ...
    <div class="mx-auto max-w-[1900px] space-y-6">
            ...
            <div class="flex flex-col gap-6 px-6 py-7 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-800 ring-1 ring-amber-200">
                    ...
                    <p class="mt-2 text-sm text-slate-600">
                        Distribution Batch #{{ $batch->id }} submitted by {{ $batch->creator?->name ?? 'TSSD Unit' }}.
                    </p>
                </div>
        ...
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @foreach([
                ['Distribution Batch', 'Batch #'.$batch->id],
                ['Purchase Order', $purchaseOrder?->po_number ?? '—'],
                ['Supplier', $purchaseOrder?->supplier?->supplier_name ?? '—'],
                ['Total Provinces', number_format($allocations->count())],
            ] as [$label, $value])
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-6 py-5 sm:px-7">
                ...
                <p class="mt-1 text-sm text-slate-500">
                    Review the consolidated PPE quantities assigned by TSSD to every provincial office in this distribution batch.
                </p>
            </div>
                ...
                <table class="min-w-[1500px] w-full border-separate border-spacing-0">
                    <thead>
                        <tr class="text-xs font-bold uppercase tracking-wide text-white">
                            <th rowspan="2" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-left">Province</th>
                            <th rowspan="2" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-center">Delivery Date</th>
                            <th rowspan="2" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-left">Place of Delivery</th>
                            <th colspan="3" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-center">Long Sleeves</th>
                            <th rowspan="2" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-center">Bucket Hat</th>
                            <th colspan="3" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-center">Rubber Boots</th>
                            <th rowspan="2" class="border-b border-r border-slate-300 bg-[#970C13] px-5 py-4 text-center">Gloves</th>
                            <th rowspan="2" class="border-b border-slate-300 bg-[#970C13] px-5 py-4 text-center">Mask</th>
                        </tr>

                        <tr class="text-[11px] font-bold uppercase">
                            @foreach(['M', 'L', 'Total', 'US9', 'US10', 'Total'] as $label)
                                <th class="border-b border-r border-slate-300 bg-[#DF979B] px-4 py-3 text-center text-[#641D21]">
                                    {{ $label }}
                                </th>
                            @endforeach
                        </tr>
                    ...
                    <tbody>
                        @forelse($allocations as $allocation)
                            ...
                            <tr class="transition hover:bg-slate-50">
                                <td class="border-b border-r border-slate-200 px-5 py-4 font-bold uppercase text-slate-900">
                                    {{ $allocation->province?->name ?? '—' }}
                                </td>
                                <td class="border-b border-r border-slate-200 px-5 py-4 text-center text-sm text-slate-600">
                                    {{ $allocation->scheduled_delivery_date?->format('M d, Y') ?? '—' }}
                                </td>
                                <td class="min-w-56 border-b border-r border-slate-200 px-5 py-4 text-sm text-slate-600">
                                    {{ $allocation->place_of_delivery ?: '—' }}
                                </td>
                                <td class="border-b border-r border-slate-200 px-4 py-4 text-center">{{ number_format($q['ls_m']) }}</td>
                                <td class="border-b border-r border-slate-200 px-4 py-4 text-center">{{ number_format($q['ls_l']) }}</td>
                                <td class="border-b border-r border-slate-200 bg-[#DF979B]/10 px-4 py-4 text-center font-bold text-[#970C13]">{{ number_format($q['ls_m'] + $q['ls_l']) }}</td>
                                <td class="border-b border-r border-slate-200 px-4 py-4 text-center">{{ number_format($q['bucket']) }}</td>
                                <td class="border-b border-r border-slate-200 px-4 py-4 text-center">{{ number_format($q['boots_9']) }}</td>
                                <td class="border-b border-r border-slate-200 px-4 py-4 text-center">{{ number_format($q['boots_10']) }}</td>
                                <td class="border-b border-r border-slate-200 bg-[#DF979B]/10 px-4 py-4 text-center font-bold text-[#970C13]">{{ number_format($q['boots_9'] + $q['boots_10']) }}</td>
                                <td class="border-b border-r border-slate-200 px-4 py-4 text-center">{{ number_format($q['gloves']) }}</td>
                                <td class="border-b border-slate-200 px-4 py-4 text-center">{{ number_format($q['mask']) }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <form action="{{ route('supply.call-offs.review', $batch) }}"
              method="POST"
            ...
            <div class="grid grid-cols-1 gap-6 p-6 lg:grid-cols-2 sm:p-7">
                <div>
                    ...
                    <input type="text"
                           ...
                           value="{{ old('call_off_number') }}"
                           required
                           maxlength="100"
                           placeholder="Example: CO-2026-0001"
                           class="w-full rounded-xl border-slate-300 uppercase focus:border-[#970C13] focus:ring-[#970C13]">
                </div>
