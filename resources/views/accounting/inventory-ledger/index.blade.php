<x-po_dashboard_layout title="Inventory Movement History">
    ...
    <div class="mx-auto max-w-[2000px] space-y-6">
                ...
                <div class="grid grid-cols-2 gap-3
                    ...
                    <div
                        ...
                        <p class="mt-1 text-sm font-bold
                                   text-slate-900">
                            {{ $selectedProvince?->name ?? 'All Offices' }}
                        </p>
                    </div>
            ...
            @foreach ($summaryCards as $card)
                <article
                    class="group rounded-2xl
                           ...
                           hover:shadow-md">
                    <div class="mb-4 h-1 w-10 rounded-full
        ...
        <section class="rounded-3xl border border-slate-200
            ...
            <form method="GET" action="{{ route('accounting.inventory-ledger.index') }}"
                ...
                <div class="xl:col-span-2">
                    ...
                    <select id="province_id" name="province_id"
                        ...
                        @foreach ($provinces as $province)
                            <option value="{{ $province->id }}" @selected($provinceId === (int) $province->id)>
                                {{ $province->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="xl:col-span-3">
                    ...
                    <select id="province_distribution_id" name="province_distribution_id"
                        class="w-full rounded-xl
                               border-slate-300
                               focus:border-[#970C13]
                               focus:ring-[#970C13]">
                        ...
                        @foreach ($callOffAllocations as $allocation)
                            @php
                                $filterCallOff = $allocation->distributionBatch?->callOff;

                                $filterSupplier = $allocation->distributionBatch?->purchaseOrder?->supplier;
                            @endphp
        ...
        <section
            ...
            @else
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[3500px]
                        ...
                        <thead>
                            ...
                            <tr class="bg-[#970C13]
                                       text-white">
                                @foreach (['#970C13', '#C51017', '#641D21'] as $sectionColor)
                                    <th colspan="2"
                                        class="border-r
                                               border-white/20
                                               px-3 py-3
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach ($pageRows as $index => $row)
                                @php
                                    ...
                                    $currentProvinceId = (int) ($row['province_id'] ?? 0);

                                    $previousProvinceId = (int) ($previousRow['province_id'] ?? 0);

                                    $isNewCallOff =
                                        $index === 0 ||
                                        $currentProvinceId !== $previousProvinceId ||
                                        $currentAllocationId !== $previousAllocationId;

                                    $deliveryReceipts = $row['delivery_receipts'] ?? collect();
                                @endphp
            ...
            <div
               ...
               lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Read-only view of the PPE currently available
                        under the Supply Unit.
                    </p>
                </div>

                <div class="rounded-2xl border border-[#DF979B]
                   bg-[#DF979B]/10 px-6 py-4">
                    ...
                    <p class="mt-1 text-3xl font-bold
                       text-[#641D21]">
                        {{ number_format($supplyInventoryTotal) }}
                    </p>
                </div>
            </div>
                ...
                <table class="w-full min-w-[900px] border-collapse">
                    <thead class="bg-[#641D21] text-white">
                        <tr class="text-xs font-bold uppercase
                           tracking-wide">
                            <th class="border border-[#7f3539] px-5 py-4">
                                No.
                            </th>

                            <th class="border border-[#7f3539]
                               px-5 py-4 text-left">
                                PPE Item
                            </th>
                    ...
                    <tbody>
                        @forelse($supplyInventory as $inventoryRow)
                            <tr class="transition hover:bg-slate-50">
                                ...
                                <td
                                    class="border border-slate-200
                                   px-5 py-4 text-slate-600">
                                    {{ $inventoryRow->unit_of_measurement ?: '—' }}
                                </td>
                                ...
                                <td
                                   ...
                                   text-lg font-bold
                                   text-[#641D21]">
                                    {{ number_format((int) $inventoryRow->quantity) }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5"
                                   ...
                                   text-slate-500">
                                    No Supply Unit inventory records found.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
