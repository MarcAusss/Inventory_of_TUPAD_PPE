<x-po_dashboard_layout title="Edit Supplier">
    <div class="mx-auto max-w-[1900px] space-y-6">
        <section class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="flex flex-col gap-6 px-6 py-7 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-4 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Edit Supplier</h1>
                    <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Update the supplier's company, contact,
        ...
        <form action="{{ route('supply.suppliers.update', $supplier) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-6 py-5 sm:px-7">
                    <p class="text-xs font-bold uppercase tracking-[0.16em] text-[#2D94BE]">Supplier information</p>
            ...
            <section
                ...
                <button type="submit"
                    class="inline-flex justify-center rounded-xl bg-[#339DCB] px-7 py-3 text-sm font-bold text-white hover:bg-[#2D94BE]">Update
                    Supplier</button>
            </section>
        </form>
    </div>
