    ...
    <div class="mx-auto max-w-[1900px] space-y-6">
        <section class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="flex flex-col gap-6 px-6 py-7 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-3xl text-sm leading-6 text-[#36566E]">Create, review, update, and manage Purchase Orders used as the source of PPE inventory and provincial Call-Off allocations.</p>
                </div>
                <a href="{{ route('supply.purchase-orders.create') }}" class="inline-flex items-center justify-center rounded-xl bg-[#339DCB] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#2D94BE]">+ New Purchase Order</a>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <form method="GET" action="{{ route('supply.purchase-orders.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="flex-1">
                    ...
                    <input type="search" id="search" name="search" value="{{ $search }}" placeholder="Search PO number, supplier, NEFA number, or status..." class="w-full rounded-xl border-slate-300 focus:border-[#339DCB] focus:ring-[#339DCB]">
                </div>
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-[#339DCB] px-6 py-3 text-sm font-bold text-white transition hover:bg-[#2D94BE]">Search</button>
                @if($search !== '')
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-6 py-5 sm:px-7">
                <p class="text-xs font-bold uppercase tracking-[0.16em] text-[#2D94BE]">Supply procurement records</p>
                        ...
                        @forelse($purchaseOrders as $po)
                            ...
                            <tr class="transition hover:bg-slate-50">
                                ...
                                <td class="whitespace-nowrap px-6 py-5">
                                    <a href="{{ route('supply.purchase-orders.show', $po) }}" class="font-semibold text-[#143A52] hover:underline">{{ $po->po_number }}</a>
                                    @if($po->nefa_number)<div class="mt-1 text-xs text-slate-400">NEFA: {{ $po->nefa_number }}</div>@endif
                                </td>
                                <td class="min-w-56 px-6 py-5 text-sm text-[#36566E]">{{ $po->supplier?->supplier_name ?? '—' }}</td>
                                <td class="whitespace-nowrap px-6 py-5 text-sm text-[#36566E]">{{ optional($po->po_date)->format('M d, Y') ?? '—' }}</td>
                                <td class="whitespace-nowrap px-6 py-5 text-right text-base font-bold text-[#2D94BE]">₱{{ number_format($po->total_amount, 2) }}</td>
                                ...
                                <td class="whitespace-nowrap px-6 py-5 text-center">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        ...
                                        <a href="{{ route('supply.purchase-orders.edit', $po) }}" class="inline-flex items-center justify-center rounded-lg bg-[#339DCB] px-4 py-2 text-sm font-bold text-white transition hover:bg-[#2D94BE]">Edit</a>
