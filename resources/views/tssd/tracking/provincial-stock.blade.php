    ...
    <div class="mx-auto max-w-[1800px] space-y-6">
        <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Provincial Office Remaining PPE Stock</h1>
                    <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                        Consolidated live balance of PPE currently held by every Provincial Office. TSSD can use this page to identify low stock, remaining inventory, and stock concentration by province.
                    </p>
                </div>
                <div class="rounded-2xl bg-[#F2F8FB] px-5 py-4 text-right ring-1 ring-[#B7D6E6]">
                    ...
                    <p class="text-xs text-slate-500">PPE units across {{ number_format($trackedProvinceCount) }} province(s)</p>
                </div>
            </div>
        </section>
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <form method="GET" action="{{ route('tssd.tracking.provincial-stock') }}"
                ...
                <a href="{{ route('tssd.tracking.provincial-stock') }}" class="rounded-xl border border-slate-300 bg-white px-5 py-3 text-center text-sm font-bold text-slate-700 hover:bg-slate-50">Reset</a>
            </form>
                    ...
                    <thead>
                        <tr>
                            ...
                            @foreach ($ppeHeaderGroups as $group)
                                ...
                                @endif
                            @endforeach
                            <th rowspan="2" class="min-w-24 px-5 py-4 text-center">Total</th>
                        </tr>
                        <tr>
                            @foreach ($ppeHeaderGroups as $group)
                                @if ($group['grouped'])
                        ...
                        @forelse ($visibleProvinces as $province)
                            <tr class="hover:bg-[#F7FBFD]">
                                ...
                                @foreach ($items as $item)
                                    @php($quantity = (int) ($province->tracking_quantities[$item->id] ?? 0))
                                    <td class="px-4 py-4 text-center font-semibold {{ $quantity <= 0 ? 'text-red-700' : 'text-slate-900' }}">
                                        {{ number_format($quantity) }}
                                    </td>
                                @endforeach
                                <td class="px-5 py-4 text-center text-base font-black text-[#2E628D]">{{ number_format((int) $province->tracking_total) }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="{{ $items->count() + 2 }}" class="px-6 py-14 text-center text-slate-500">No Provincial Office stock matched the current filter.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
